...
const Newsletter = ({ toggleSidebar, isSidebarOpen, mode, toggleMode, onLogout }) => {
    const { user } = useUser();
    ...
    return (
        ...
        >
            <div className="flex flex-col justify-between p-8 h-full">  {/* Add h-full to ensure full height */}
                <div className="mb-4">
                    <Icon icon="fluent-mdl2:news" className="text-5xl text-sky-500"/>
                </div>
                <h3 className="main-gray font-bold text-xl mb-4">NDSI Newsletter</h3>
                <p className="mb-6">Stay updated with the latest news, reports, and industry insights.</p>
                <button
